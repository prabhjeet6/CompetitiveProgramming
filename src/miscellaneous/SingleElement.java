...
public class SingleElement {
    ...
    public static void main(String asd[]) throws Exception {
        ...
        System.out.println(binarySearchReturnsOnlySingleElement(arr));

    }

    static Long binarySearchReturnsOnlySingleElement(long arr[]) {
        int low = 0, high = arr.length - 1, mid = 0, firstOccurrence = 0, secondOccurrence = 0;
        if (arr.length == 1)
            return arr[0];
        while (low <= high) {
            mid = (low + high) / 2;
